  ...
  <div class="friend">
    <div class="site-content">
      ...
      <div class="statement">
        ...
        <quote>
          <p>站点名称：蘑菇博客</p>
          <p>站点链接：http://demoweb.moguit.cn/</p>
          <p>简短描述：陌溪开发</p>
        </quote>
        <p>接入成功后将会以邮件的方式通知。</p>
      </div>
      ...
      <div class="friend-list animate">
        <div class="friend-item" v-for="item in list" :key="item.id"><a target="_blank" :href="item.link">
          <div class="site-name">{{ item.name }}</div>
          <div class="site-detail">{{ item.descp }}</div>
        </a></div>
      </div>
    </div>
  </div>
